...
f_x = np.gradient(reg_data, axis=0)/dx
f_y = np.gradient(reg_data, axis=1)/dy
f_xx = np.gradient(f_x, axis=0)/dx
f_yy = np.gradient(f_y, axis=1)/dy
...
partial = np.delete(partial, np.where(np.absolute(f_t_col)<0.001) ,0)
f_t_col = np.delete(f_t_col, np.where(np.absolute(f_t_col)<0.001))
